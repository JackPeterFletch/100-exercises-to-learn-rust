...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    //         TitleError { message } => panic!("{}", message),
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TitleError { message: "Title cannot be empty".to_string() });
        }
        if title.len() > 50 {
            return Err(TitleError { message: "Title cannot be longer than 50 characters".to_string() });
        }
        if description.is_empty() {
            return Err(DescriptionError { message: "Description cannot be empty".to_string() });
        }
        if description.len() > 500 {
            return Err(DescriptionError { message: "Description cannot be longer than 500 characters".to_string()});
        }
